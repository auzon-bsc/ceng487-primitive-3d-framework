...
def DrawGLScene():
	...
	glRotatef(r_angle,1.0,1.0,0.0);	# Rotate The Object On It's X and Y Axis
	...
	if object_to_draw == "SPHERE":
		...
		for poly in sphere.poly_arr:
			...
			for j in range(len(poly)):
				glColor3f(colors[j][0], colors[j][1], colors[j][2])		# Change color of each vertice of a polygon
		...
		for poly in cylinder.poly_arr:
			...
			for i in range(len(poly)):
				glColor3f(colors[i%3][0], colors[i%3][1], colors[i%3][2])		# Change color of each vertice of a polygon
		...
		for square in cube:
			vertice_arr = square.transform()		# Vertice array of all polygons of the square
			# For each 4 vertices of a polygon
			for i in range(len(vertice_arr) // 4):
				glBegin(GL_QUADS)
				
				glColor3f(1,0,0)
				glVertex3f(vertice_arr[i*4].x,vertice_arr[i*4].y,vertice_arr[i*4].z)		# Left top vertice
				
				glColor3f(0,1,0)
				glVertex3f(vertice_arr[i*4+1].x,vertice_arr[i*4+1].y,vertice_arr[i*4+1].z)		# Right top vertice
				
				glColor3f(0,0,1)
				glVertex3f(vertice_arr[i*4+2].x,vertice_arr[i*4+2].y,vertice_arr[i*4+2].z)		# Right bottom vertice
				
				glColor3f(0.5,0.5,0.5)
				glVertex3f(vertice_arr[i*4+3].x,vertice_arr[i*4+3].y,vertice_arr[i*4+3].z)		# Left bottom vertice
